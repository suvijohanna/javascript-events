...
restaurants.sort(function (a, b) {
  if (a.name < b.name) return -1; // Jos a tulee ennen b:tä, lajitellaan ensin
  if (a.name > b.name) return 1; // Jos b tulee ennen a:ta, lajitellaan ensin
  return 0;
});
...
for (const restaurant of restaurants) {
  ...
  rivi.addEventListener('click', function () {
    ...
    if (currentHighlight && currentHighlight !== rivi) {
      currentHighlight.classList.remove('highlight');
    }
    rivi.classList.toggle('highlight');

    if (rivi.classList.contains('highlight')) {
      modal.innerHTML = `
        ...
        <button id="closeBtn">Sulje ikkuna</button>
      `;
      modal.showModal();
